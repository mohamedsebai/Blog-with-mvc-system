    ...
    final public function msg(){
       ...
       if($_SERVER['REQUEST_METHOD'] == "POST"){
         ...
         $file_extension = $this->file_extension($img_name);
           ...
           if($img_error == 4){
             $target_file = $this->auth->check_if_email_existis( Session::get('email') )['row']['profile_img'];
           }
           if($img_error !== 4){
             $random_img_name = rand(0, getrandmax()) . $img_name;
             $dir_path =  $_SERVER['DOCUMENT_ROOT'] . '/mvc_oop/admin/public/img/post/';
             $target_file = $dir_path . basename($random_img_name);
         ...
         if(empty($data['emailError']) && empty($data['fullnameError']) && empty($data['imageError'])){
            $updated_at = date('Y:m:d H:i:s');
                 if($this->auth->update_user($data['fullname'], $data['email'], $target_file, $data['country'], $data['gender'],
